...
static void CreateRenderTarget(IDXGISwapChain* pSwapChain)
{
	...
	if (pBackBuffer) {
		...
		if (!g_pDevice) {
			std::cerr << "This is not funny!" << std::endl;
			return;
		}
		...
		if (FAILED(g_pDevice->CreateRenderTargetView(pBackBuffer, &desc, &g_pRenderTargetView))) {
			...
			if (FAILED(g_pDevice->CreateRenderTargetView(pBackBuffer, &desc, &g_pRenderTargetView))) {
				...
				if (FAILED(g_pDevice->CreateRenderTargetView(pBackBuffer, NULL, &g_pRenderTargetView))) {
					std::cerr<<"CreateRenderTargetView failure!" << std::endl;
				}
			}
		}
...
static void CleanupRenderTarget() {
	std::cerr << "DX11 CleanupRenderTarget()" <<std::endl;
...
static void CleanupDevice() {
	std::cerr << "DX11 CleanupDevice()" << std::endl;
...
static void RenderImGui(IDXGISwapChain* pSwapChain) {
	...
	if (!ImGui::GetIO().BackendRendererUserData) {
		...
		if (!SUCCEEDED(pSwapChain->GetDevice(IID_PPV_ARGS(&g_pDevice)))) {
			std::cerr << "Couldnt get device?!???" << std::endl;
			return;
		}
	...
	{
		MainMenu::showMenu = !MainMenu::showMenu;
		ImGui::GetIO().MouseDrawCursor =  MainMenu::showMenu;
...
bool D3D11Setup()
{
	...
	if (!CreateDevice(g_cheat->wnd_proc_hooks->hwndWindow)) {
		std::cerr << "Unable to create dx11 device!" << std::endl;
		return false;
	}
