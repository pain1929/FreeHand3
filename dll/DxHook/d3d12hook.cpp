...
namespace d3d12hook {
    ...
    long __fastcall hookPresent1D3D12(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags, const DXGI_PRESENT_PARAMETERS* pParams) {
        ...
        if (!gShutdown) {
            ...
            if (gOverlayFence && gFenceEvent)
            {
                const UINT64 completed = gOverlayFence->GetCompletedValue();

                // GPU 落后太多时暂时跳过渲染
                if (completed + 2 < gOverlayFenceValue)
                {
                    canRender = false;
                }
                else if (completed < gOverlayFenceValue)
                {
                    // GPU 仅略微落后 -> 尝试有限等待
                    HRESULT hr = gOverlayFence->SetEventOnCompletion(gOverlayFenceValue, gFenceEvent);
                    if (SUCCEEDED(hr))
                    {
                        constexpr DWORD waitTimeoutMs = 5; // 微等待 (非阻塞级别)
                        DWORD waitRes = WaitForSingleObject(gFenceEvent, waitTimeoutMs);
                        if (waitRes == WAIT_TIMEOUT)
                        {
                            canRender = false; // GPU 还没追上，跳过本帧渲染
                        }
                        else if (waitRes != WAIT_OBJECT_0)
                        {
                            canRender = false; // 异常
                    ...
                    {
                        canRender = false; // SetEventOnCompletion 失败
                    }
                }
            }
            else
        ...
        __try {
            ...
            //DebugLog("Present crashed, skipping this frame\n");
            return S_OK;
        }
    }
