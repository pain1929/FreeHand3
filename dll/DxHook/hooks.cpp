...
namespace hooks {
    ...
    void Init() {
        ...
        if (mh != MH_OK) {
            throw std::runtime_error(xorstr_("[hooks] MH_CreateHook Present failed") + std::string(MH_StatusToString(mh)));
        }
        ...
        if (mh != MH_OK) {
            throw std::runtime_error(xorstr_("[hooks] MH_CreateHook Present1 failed")+ std::string(MH_StatusToString(mh)));
        }
        ...
        if (mh != MH_OK) {
            throw std::runtime_error(xorstr_("[hooks] MH_CreateHook pResizeBuffersTarget failed")+ std::string(MH_StatusToString(mh)));
        }


        // --- Hook ExecuteCommandLists ---
        auto cqVTable = *(void ***)pCommandQueue.Get();
        ...
        if (mh != MH_OK) {
            throw std::runtime_error(xorstr_("[hooks] MH_CreateHook hookExecuteCommandListsD3D12 failed")+ std::string(MH_StatusToString(mh)));
        }

        // --- Enable all hooks ---
        mh = MH_EnableHook(MH_ALL_HOOKS);
        if (mh != MH_OK) {
           throw std::runtime_error(xorstr_("dx12 hook error")+ std::string(MH_StatusToString(mh)));
        }
